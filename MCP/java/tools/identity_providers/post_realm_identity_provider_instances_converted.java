/**
 * MCP Server function for Create a new identity provider
 */
...
class Post_Realm_Identity_Provider_InstancesMCPTool {
    
    public static Function<MCPServer.MCPRequest, MCPServer.MCPToolResult> getPost_Realm_Identity_Provider_InstancesHandler(MCPServer.APIConfig config) {
        return (request) -> {
            try {
        ...
        if (args.containsKey("config")) {
            queryParams.add("config=" + args.get("config"));
                ...
                String url = config.getBaseUrl() + "/api/v2/post_realm_identity_provider_instances" + queryString;
    ...
    public static MCPServer.Tool createPost_Realm_Identity_Provider_InstancesTool(MCPServer.APIConfig config) {
        ...
        MCPServer.ToolDefinition definition = new MCPServer.ToolDefinition(
            "post_realm_identity_provider_instances",
            "Create a new identity provider",
            parameters
        );
        
        return new MCPServer.Tool(definition, getPost_Realm_Identity_Provider_InstancesHandler(config));
    }
