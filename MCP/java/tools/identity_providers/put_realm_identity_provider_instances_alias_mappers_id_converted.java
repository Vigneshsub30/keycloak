/**
 * MCP Server function for Update a mapper for the identity provider
 */
...
class Put_Realm_Identity_Provider_Instances_Alias_Mappers_IdMCPTool {
    
    public static Function<MCPServer.MCPRequest, MCPServer.MCPToolResult> getPut_Realm_Identity_Provider_Instances_Alias_Mappers_IdHandler(MCPServer.APIConfig config) {
        return (request) -> {
            try {
        ...
        if (args.containsKey("config")) {
            queryParams.add("config=" + args.get("config"));
                ...
                String url = config.getBaseUrl() + "/api/v2/put_realm_identity_provider_instances_alias_mappers_id" + queryString;
    ...
    public static MCPServer.Tool createPut_Realm_Identity_Provider_Instances_Alias_Mappers_IdTool(MCPServer.APIConfig config) {
        ...
        MCPServer.ToolDefinition definition = new MCPServer.ToolDefinition(
            "put_realm_identity_provider_instances_alias_mappers_id",
            "Update a mapper for the identity provider",
            parameters
        );
        
        return new MCPServer.Tool(definition, getPut_Realm_Identity_Provider_Instances_Alias_Mappers_IdHandler(config));
    }
