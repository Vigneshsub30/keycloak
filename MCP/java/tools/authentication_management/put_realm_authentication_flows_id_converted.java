/**
 * MCP Server function for Update an authentication flow
 */
...
class Put_Realm_Authentication_Flows_IdMCPTool {
    
    public static Function<MCPServer.MCPRequest, MCPServer.MCPToolResult> getPut_Realm_Authentication_Flows_IdHandler(MCPServer.APIConfig config) {
        return (request) -> {
            try {
        ...
        if (args.containsKey("authenticationExecutions")) {
            queryParams.add("authenticationExecutions=" + args.get("authenticationExecutions"));
                ...
                String url = config.getBaseUrl() + "/api/v2/put_realm_authentication_flows_id" + queryString;
    ...
    public static MCPServer.Tool createPut_Realm_Authentication_Flows_IdTool(MCPServer.APIConfig config) {
        ...
        MCPServer.ToolDefinition definition = new MCPServer.ToolDefinition(
            "put_realm_authentication_flows_id",
            "Update an authentication flow",
            parameters
        );
        
        return new MCPServer.Tool(definition, getPut_Realm_Authentication_Flows_IdHandler(config));
    }
