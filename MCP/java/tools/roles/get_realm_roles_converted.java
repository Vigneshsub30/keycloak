...
class Get_Realm_RolesMCPTool {
    
    public static Function<MCPServer.MCPRequest, MCPServer.MCPToolResult> getGet_Realm_RolesHandler(MCPServer.APIConfig config) {
        return (request) -> {
            try {
        ...
        if (args.containsKey("briefRepresentation")) {
            queryParams.add("briefRepresentation=" + args.get("briefRepresentation"));
                ...
                String url = config.getBaseUrl() + "/api/v2/get_realm_roles" + queryString;
    ...
    public static MCPServer.Tool createGet_Realm_RolesTool(MCPServer.APIConfig config) {
        ...
        return new MCPServer.Tool(definition, getGet_Realm_RolesHandler(config));
    }
