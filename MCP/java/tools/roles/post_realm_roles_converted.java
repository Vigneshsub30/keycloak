...
class Post_Realm_RolesMCPTool {
    
    public static Function<MCPServer.MCPRequest, MCPServer.MCPToolResult> getPost_Realm_RolesHandler(MCPServer.APIConfig config) {
        return (request) -> {
            try {
        ...
        if (args.containsKey("attributes")) {
            queryParams.add("attributes=" + args.get("attributes"));
                ...
                String url = config.getBaseUrl() + "/api/v2/post_realm_roles" + queryString;
    ...
    public static MCPServer.Tool createPost_Realm_RolesTool(MCPServer.APIConfig config) {
        ...
        return new MCPServer.Tool(definition, getPost_Realm_RolesHandler(config));
    }
