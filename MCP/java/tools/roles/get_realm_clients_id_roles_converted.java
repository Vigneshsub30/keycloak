/**
 * MCP Server function for Get all roles for the realm or client
 */
...
class Get_Realm_Clients_Id_RolesMCPTool {
    
    public static Function<MCPServer.MCPRequest, MCPServer.MCPToolResult> getGet_Realm_Clients_Id_RolesHandler(MCPServer.APIConfig config) {
        return (request) -> {
            try {
        ...
        if (args.containsKey("briefRepresentation")) {
            queryParams.add("briefRepresentation=" + args.get("briefRepresentation"));
                ...
                String url = config.getBaseUrl() + "/api/v2/get_realm_clients_id_roles" + queryString;
    ...
    public static MCPServer.Tool createGet_Realm_Clients_Id_RolesTool(MCPServer.APIConfig config) {
        ...
        MCPServer.ToolDefinition definition = new MCPServer.ToolDefinition(
            "get_realm_clients_id_roles",
            "Get all roles for the realm or client",
            parameters
        );
        
        return new MCPServer.Tool(definition, getGet_Realm_Clients_Id_RolesHandler(config));
    }
