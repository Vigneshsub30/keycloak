/**
 * MCP Server function for Import a realm   Imports a realm from a full representation of that realm.
 */
...
class PostMCPTool {
    ...
    public static Function<MCPServer.MCPRequest, MCPServer.MCPToolResult> getPostHandler(MCPServer.APIConfig config) {
        return (request) -> {
            try {
        ...
        if (args.containsKey("webAuthnPolicyPasswordlessAcceptableAaguids")) {
            queryParams.add("webAuthnPolicyPasswordlessAcceptableAaguids=" + args.get("webAuthnPolicyPasswordlessAcceptableAaguids"));
                ...
                String url = config.getBaseUrl() + "/api/v2/post" + queryString;
    ...
    public static MCPServer.Tool createPostTool(MCPServer.APIConfig config) {
        ...
        return new MCPServer.Tool(definition, getPostHandler(config));
    }
