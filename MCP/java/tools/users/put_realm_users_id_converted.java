/**
 * MCP Server function for Update the user
 */
...
class Put_Realm_Users_IdMCPTool {
    
    public static Function<MCPServer.MCPRequest, MCPServer.MCPToolResult> getPut_Realm_Users_IdHandler(MCPServer.APIConfig config) {
        return (request) -> {
            try {
        ...
        if (args.containsKey("disableableCredentialTypes")) {
            queryParams.add("disableableCredentialTypes=" + args.get("disableableCredentialTypes"));
                ...
                String url = config.getBaseUrl() + "/api/v2/put_realm_users_id" + queryString;
    ...
    public static MCPServer.Tool createPut_Realm_Users_IdTool(MCPServer.APIConfig config) {
        ...
        return new MCPServer.Tool(definition, getPut_Realm_Users_IdHandler(config));
    }
