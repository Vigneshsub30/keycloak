/**
 * MCP Server function for create or add a top level realm groupSet or create child.
 */
...
class Post_Realm_GroupsMCPTool {
    
    public static Function<MCPServer.MCPRequest, MCPServer.MCPToolResult> getPost_Realm_GroupsHandler(MCPServer.APIConfig config) {
        return (request) -> {
            try {
        ...
        if (args.containsKey("subGroups")) {
            queryParams.add("subGroups=" + args.get("subGroups"));
                ...
                String url = config.getBaseUrl() + "/api/v2/post_realm_groups" + queryString;
    ...
    public static MCPServer.Tool createPost_Realm_GroupsTool(MCPServer.APIConfig config) {
        ...
        return new MCPServer.Tool(definition, getPost_Realm_GroupsHandler(config));
    }
