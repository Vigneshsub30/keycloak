/**
 * MCP Server function for Update group, ignores subgroups.
 */
...
class Put_Realm_Groups_IdMCPTool {
    
    public static Function<MCPServer.MCPRequest, MCPServer.MCPToolResult> getPut_Realm_Groups_IdHandler(MCPServer.APIConfig config) {
        return (request) -> {
            try {
        ...
        if (args.containsKey("realmRoles")) {
            queryParams.add("realmRoles=" + args.get("realmRoles"));
                ...
                String url = config.getBaseUrl() + "/api/v2/put_realm_groups_id" + queryString;
    ...
    public static MCPServer.Tool createPut_Realm_Groups_IdTool(MCPServer.APIConfig config) {
        ...
        return new MCPServer.Tool(definition, getPut_Realm_Groups_IdHandler(config));
    }
