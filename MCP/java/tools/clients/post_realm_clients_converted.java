/**
 * MCP Server function for Create a new client   Client’s client_id must be unique!
 */
...
class Post_Realm_ClientsMCPTool {
    
    public static Function<MCPServer.MCPRequest, MCPServer.MCPToolResult> getPost_Realm_ClientsHandler(MCPServer.APIConfig config) {
        return (request) -> {
            try {
        ...
        if (args.containsKey("webOrigins")) {
            queryParams.add("webOrigins=" + args.get("webOrigins"));
                ...
                String url = config.getBaseUrl() + "/api/v2/post_realm_clients" + queryString;
    ...
    public static MCPServer.Tool createPost_Realm_ClientsTool(MCPServer.APIConfig config) {
        ...
        return new MCPServer.Tool(definition, getPost_Realm_ClientsHandler(config));
    }
