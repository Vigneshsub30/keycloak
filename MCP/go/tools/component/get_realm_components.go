...
func Get_realm_componentsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/%s/components%s", cfg.BaseURL, queryString)
...
func CreateGet_realm_componentsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_realm_components",
		mcp.WithDescription(""),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Get_realm_componentsHandler(cfg),
	}
}
