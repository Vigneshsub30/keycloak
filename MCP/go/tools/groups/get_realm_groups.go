...
func Get_realm_groupsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/%s/groups%s", cfg.BaseURL, queryString)
...
func CreateGet_realm_groupsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_realm_groups",
	...
	return models.Tool{
		Definition: tool,
		Handler:    Get_realm_groupsHandler(cfg),
	}
}
