...
func Get_realm_users_id_groupsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/%s/users/%s/groups%s", cfg.BaseURL, queryString)
...
func CreateGet_realm_users_id_groupsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_realm_users_id_groups",
		mcp.WithDescription(""),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Get_realm_users_id_groupsHandler(cfg),
	}
}
