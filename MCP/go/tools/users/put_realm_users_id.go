...
func Put_realm_users_idHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreatePut_realm_users_idTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("put_realm_users_id",
		mcp.WithDescription("Update the user"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Put_realm_users_idHandler(cfg),
	}
}
