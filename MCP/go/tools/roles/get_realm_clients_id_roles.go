...
func Get_realm_clients_id_rolesHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/%s/clients/%s/roles%s", cfg.BaseURL, queryString)
...
func CreateGet_realm_clients_id_rolesTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_realm_clients_id_roles",
		mcp.WithDescription("Get all roles for the realm or client"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Get_realm_clients_id_rolesHandler(cfg),
	}
}
