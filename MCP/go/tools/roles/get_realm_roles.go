...
func Get_realm_rolesHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/%s/roles%s", cfg.BaseURL, queryString)
...
func CreateGet_realm_rolesTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_realm_roles",
	...
	return models.Tool{
		Definition: tool,
		Handler:    Get_realm_rolesHandler(cfg),
	}
}
